--modified version for SpiCe contest
...
--   local x = self:encode_string('bcd'):view(1, -1)
--   local T0 = x:size(2)
--   scores = self:forward(x)[{{}, {T0, T0}}]
--   local probs = torch.div(scores, 1.0):double():exp():squeeze()
--   probs:div(torch.sum(probs))
--   local next_chars, next_indices = torch.topk(probs, 10)
--   print("got next 10 char indices for FOO:")
--   print(next_indices)
--   -- print(probs)

--   self:resetStates()

--   x = self:encode_string("ghibcdebhdddbe"):view(1, -1)
--   T0 = x:size(2)
--   scores = self:forward(x)[{{}, {T0, T0}}]
--   probs = torch.div(scores, 1.0):double():exp():squeeze()
--   probs:div(torch.sum(probs))
--   local next_chars, next_indices = torch.topk(probs, 10)
--   print("got next 10 char indices for FOO:")
--   print(next_indices)
  ...
  for t = first_t, T do
    if sample == 0 then
      ...
      next_char = next_char[{{}, {}, 1}]
       ...
       -- local num_samples = 5
       -- local a_bunch_of_possible_chars = torch.multinomial(probs, num_samples, true)
       -- print (a_bunch_of_possible_chars)
       -- print (torch.multinomial(probs, 1))
       -- print (probs)
       -- for i=1,5 do
       --   n_char = self.idx_to_token[i]
       --   if string.byte (n_char) == 10 then -- new line character
       --    print ('a')
       --   else
       --    print (n_char)
       --   end
       ...
       local next_chars, next_indices = torch.topk(probs,  5, 1,true, true)
       -- local next_chars, next_indices = torch.topk(probs,  5, true, true)
       -- print (next_indices)
       for i = 1,5 do
        -- print (next_indices[i])
        n_char = self.idx_to_token [next_indices[i]]
        if string.byte (n_char) == 10 then -- new line character
          print ('a')
         else
          print (n_char)
